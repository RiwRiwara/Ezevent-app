    ...
    return (
      <View w="$full" alignItems="center">
        <Link href={`/event/${myevent.event_id}`} push>
          <HStack w="$full" h={150}>
            <HStack justifyContent="space-between" alignItems="center" w="$1/3" px={5}>
              <VStack alignItems="center" w="$full">
                <Image
                  ...
                  borderWidth="$2"
                  borderColor="$gray7"
                  source={{ uri: myevent.getBannerImage }}
                />
              </VStack>
            </HStack>
            <HStack w="$2/3" py={10}>
              <VStack justifyContent="space-between" w="$full" px={5}>
                ...
                <Text fontSize="$paragraph" fontWeight="$bold" color="$neutral9">
                  {myevent.event_name.length > 20 ? `${myevent.event_name.substring(0, 20)}...` : myevent.event_name}
                </Text>
                <HStack justifyContent="flex-end">
                      <ButtonWhiteSet title={whiteBtn} status={status} event_participant_id={participantId} />
                      <ActionButionSet title={titleBtn} color={colorBtn} event_participant_id={participantId} action={action} />
                </HStack>
              </VStack>
            </HStack>
          </HStack>
